  ...
  useEffect(() => {
    const fetchPapers = async () => {
      try {
        ...
        setPaperData(response.data || {}); // fallback to empty object
      } catch (err) {
      ...
      <Grid gutter="xl">
        ...
        <Grid.Col span={{ base: 12, md: 10 }}>
          ...
          <Text size="sm" mt="sm">
            ...
            {paperData.authors?.map(a => a.user?.name || "").join(", ") || "N/A"}
          </Text>
          ...
          <Text size="sm" mt="sm">
            <strong>Abstract:</strong> {paperData.abstract || "No abstract available."}
          </Text>
          ...
          <Text>
            <strong>Year:</strong>{" "}
            {paperData.date ? new Date(paperData.date).getFullYear() : "N/A"}
          </Text>
          <Text>
            <strong>DOI:</strong>{" "}
            {paperData.doi ? (
              <Anchor href={`https://doi.org/${paperData.doi}`} target="_blank">
                {paperData.doi}
              </Anchor>
            ) : (
              "N/A"
